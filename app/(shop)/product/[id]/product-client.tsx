...
// ✅ Props simplifiées — reçoit id directement
interface Props {
    id: string
}

// ✅ Export nommé (pas default) pour être importé dans page.tsx
    ...
    useEffect(() => {
        const loadProduct = async () => {
            try {
                ...
                const data = await getProductByIdFromSupabase(id) // ✅ params.id → id
                if (data) {
                    setProduct(data)
                                ...
                                <div className="grid grid-cols-5 gap-3 sm:grid-cols-6">
                                    {productImages.slice(0, 12).map((imageUrl, index) => (
                                        <button
                                            ...
                                            className={`relative aspect-square overflow-hidden rounded-2xl border bg-white transition-all ${activeImageIndex === index ? 'border-green-500 ring-2 ring-green-500/20' : 'border-gray-200 hover:border-green-300'
                                                }`}
