...
const productSchema = z.object({
  ...
  brand_id: z.string().trim().uuid().optional().nullable().or(z.literal('')),
...
export function sanitizeProduct(body: any) {
  ...
  return {
    ...
    specs: {
      brand: product.brand_id ? null : product.brand || null,
      tags: product.tags ? product.tags.split(',').map((t: string) => t.trim()).filter(Boolean) : null,
    },
